...
import fedelef.hockeyapp.models.GameStats;
...
public class NHLDataAccessObject {
	...
	public List<Player> getPlayers(int teamID, String teamName)
			throws IOException, InterruptedException, JSONException {

		var client = HttpClient.newHttpClient();

		var request = HttpRequest
				.newBuilder(URI.create(String.format("https://statsapi.web.nhl.com/api/v1/teams/%d/roster", teamID)))
				.header("accept", "application/json").build();
		...
		for (int i = 0; i < roster.length(); i++) {
			...
			if (!player.getJSONObject("position").getString("code").equals("G")) {
				...
				players.add(new Player(player.getJSONObject("person").getInt("id"),
						player.getJSONObject("person").getString("fullName"), player.getInt("jerseyNumber"),
						player.getJSONObject("position").getString("code"), teamName));
		...
		for (Player player : players) {
			...
			if (!player.getPosition().equals("G")) {
				...
				try {
					...
					// players on the roster may not be active - check if this is something we can
					// fix earlier!
					if (splitsArray.length() != 0) {
						statsObj = new JSONObject(splitsArray.getJSONObject(0), new String[] { "stat" })
								.getJSONObject("stat");

						player.setStats(statsObj.getInt("goals"), statsObj.getInt("games"), statsObj.getInt("assists"),
								statsObj.getInt("plusMinus"), statsObj.getDouble("shotPct"));
	...
	public void setGameStats(Player player) throws IOException, InterruptedException, JSONException {
		...
		ArrayList<GameStats> gameStats = new ArrayList<GameStats>();
